...
-------------------------------------------------------------------------------
  ...
  FUNCTION get_name (pnet_object_id IN NUMBER)
    RETURN VARCHAR2
  IS
    object_name VARCHAR2(550);
  BEGIN
    select name into object_name from pn_object_name where object_id = pnet_object_id;
    
    return object_name;
    EXCEPTION
  WHEN OTHERS THEN
    BEGIN
      BASE.LOG_ERROR('get_name', SQLCODE, SQLERRM);
      RAISE;
    END;
  END get_name;
