...
Ext.onReady(function(){
	   ...
	   sm = new xg.RowSelectionModel({
		...
		function renderWithPercent(v, params, record){
			...
			if (record.data.resourceName == gridTotalRowLabel){
       ...
       columnModel = new Ext.grid.ColumnModel([
					...
					header: untilEndOfPorjectColumnHeader,
					width: 200,
					sortable: false,					
					dataIndex: 'untilEnd',	               
      ...
      grid = new xg.EditorGridPanel({
			...
			, {
					...
					tooltip: {title: addResourceTooltipTitle , text: clickToAddResourceTooltip, width:'150'},
					iconCls: 'add',
					handler : function(){
					var person = new personRecord({
                    resource_id: combo.value,
					resourceName: getComboDisplayedValue(combo.value),
					untilEnd: '',
				...
				grid.stopEditing();
				if(combo.value == '0'){
	                extAlert(extAlertTitle, selectResourceToAddMessage, Ext.MessageBox.ERROR);
				} else if(viewedProjectId == 0){
					extAlert(extAlertTitle, projectNotExistToAllocateMessage, Ext.MessageBox.ERROR);
				} else if(isNotInList(combo.value)){
					if(grid.getStore().getCount() > 0){
						ds.insert(grid.getStore().getCount()-1, person);
					} else {
						ds.insert(0, person);
					}
				} else {
					extAlert(extAlertTitle, resourceAlreadyAddedMessage, Ext.MessageBox.ERROR);
				}
...
xgdata1 = projectAllocationData;

function saveGridValues(){	
	if(viewedProjectId == 0){
		extAlert(extAlertTitle, projectNotExistToAllocateMessage, Ext.MessageBox.ERROR);
		return false;
	}
	...
	for(var recIndex = 0; recIndex < recordsLength - 1; recIndex++){
		...
		allocationArray += '{ "resourceId" : '+ record.get('resource_id') +','
	...
	Ext.Ajax.request({
		   url: jspRootUrl+'/resource/management/allocateByProject/dummy/ForSaveAllocations',
		   params: {module: 310, allocationArray: allocationArray},
		   method: 'POST',
		   success: function(result, request){
					 resText = result.responseText.split('|&|');
					 comboList = resText[1];	
					 document.getElementById('projects').innerHTML = resText[6];
					 document.getElementById('business').value = resText[2];
					 document.getElementById('project').value = resText[3];
...
function showResult(numOfMonths){
	...
	Ext.Ajax.request({
		   url: jspRootUrl+escapeUrl('/resource/management/AllocateByProject/'+ startMonth +'&'+ startYear+'&'+ projectId +'&'+ bussinessId+'/ForFetchDataByMonths'),
		   params: {module: 310},
		   method: 'POST',
		   success: function(result, request){ 
...
function refreshGrid(){
	...
	var gridData = '[ ';
	var recordsLength = grid.getStore().getCount();
	if(grid.getStore().getAt(recordsLength-1).get('resourceName') == gridTotalRowLabel){
		recordsLength--;
	}
	for(var recIndex = 0; recIndex < recordsLength; recIndex++){
		var record = grid.getStore().getAt(recIndex);	
		gridData +='['+ record.get('resource_id') +',"'+ record.get('resourceName') +'",'+record.get('untilEnd')+',';
