...
public class WikiLayout {
	...
	void onActivate() {
		try {
			log = Logger.getLogger(WikiLayout.class);
			application_title = PropertyProvider.get("prm.global.application.title");
			personal_on = PropertyProvider.get("prm.global.header.personal.image.on");
			personal_off = PropertyProvider.get("prm.global.header.personal.image.off");
			business_on = PropertyProvider.get("prm.global.header.business.image.on");
			business_off = PropertyProvider.get("prm.global.header.business.image.off");
			project_on = PropertyProvider.get("prm.global.header.project.image.on");
			project_off = PropertyProvider.get("prm.global.header.project.image.off");
			resources_on = PropertyProvider.get("prm.global.header.resource.image.on");
			resources_off = PropertyProvider.get("prm.global.header.resource.image.off");
			errotAlertTitle = PropertyProvider.get("prm.resource.global.exterroralert.title");
		} catch (Exception ex) {
			log.error("Error occured while getting property values in WikiLayout page : "+ex.getMessage());			
		}
	}
	...
	/**
	 * @return Returns the spaceForNavbar.
	 */
	...
	/**
	 * @param spaceForNavbar The spaceForNavbar to set.
	 */
	public void setSpaceForNavbar(String spaceForNavbar) {
		this.spaceForNavbar = spaceForNavbar;
	}
