...
import net.project.admin.DocumentRepositoryRoot;
...
public class DocumentRootUpdateHandler extends Handler {
    public DocumentRootUpdateHandler(HttpServletRequest request) {
        super(request);
    }
    ...
    public String getViewName() {
        return "/servlet/AdminController/DocumentAdmin";
    }
    ...
    public Map handleRequest(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        //Find all the ids of the document roots that were submitted
        String[] docRootID = request.getParameterValues("docRootID");

        for (int i = 0; i < docRootID.length; i++) {
            String id = docRootID[i];

            DocumentRepositoryRoot root = new DocumentRepositoryRoot();
            root.setID((id.startsWith("new_") ? null : id));
            root.setPath(request.getParameter("docPath_"+id));
            //Avinash: checking for a valid path
            if(root.pathExists()==false){
            	request.setAttribute("message","prm.admin.docvoult.docRootUpdate.path.error.message");
            	this.passThruAll(model);
            	return model;
            }
            root.setActive(new Boolean(request.getParameter("isActive_"+id)).booleanValue());
