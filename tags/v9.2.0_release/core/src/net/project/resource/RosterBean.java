...
/**  
    A Roster of people for use within JSP pages.      
*/
public class RosterBean extends Roster implements Serializable
{

    /**
     * Returns the RosterBean from the session or creates one and puts it in
     * the session if there is none.
     * @param session the session from which to get the RosterBean
     * @return the RosterBean
     */
    public static RosterBean getFromSession(HttpSession session) {
        RosterBean roster = (RosterBean) session.getAttribute("roster");
        if (roster == null) {
