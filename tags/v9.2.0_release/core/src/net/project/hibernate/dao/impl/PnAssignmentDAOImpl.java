...
public class PnAssignmentDAOImpl extends AbstractHibernateAnnotatedDAO<PnAssignment, PnAssignmentPK> implements IPnAssignmentDAO {
	...
	public List<PnAssignment> getAssigmentsList(Integer[] personIds, Date startDate, Date endDate) {
		...
		try {
			String sql = "SELECT distinct a.percentAllocated, a.workComplete,  t.actualStart, t.actualFinish, a.work ,  "
                    ...
                    + " p.personId, p.firstName, p.lastName, p.displayName,  "
					+ " ps.projectId, ps.projectName, a.startDate, a.endDate  " 
					...
					+ " t.recordStatus = 'A' AND ps.recordStatus = 'A' AND a.comp_id.spaceId = shs.comp_id.childSpaceId and " + " shs.comp_id.childSpaceId = ps.projectId ";
			...
			if (startDate != null && endDate != null) {
				sql = sql + " AND ((a.startDate BETWEEN :startDate AND :endDate " 
			...
			while (results.hasNext()) {
				...
				PnTask task = new PnTask((Integer) row[5], (String) row[6], (BigDecimal) row[7], (Date) row[8], (Date) row[9], (BigDecimal) row[10], (Date) row[2], (Date) row[3]);
	...
	/* (non-Javadoc)
	 * @see net.project.hibernate.dao.IPnAssignmentDAO#getAssigmentsList(java.lang.Integer, java.lang.Integer[], java.util.Date)
	 */
	...
	public List getWorkSumByMonthForUsers(Integer[] personIds, Integer[] projectIds, Date startDate, Date endDate) {
		String sql = "SELECT p.personId, p.firstName,p.lastName, a.percentAllocated, a.work,  " 
				+ "	 t.dateStart, t.dateFinish,  ps.projectId,ps.projectName  " 
				+ " FROM PnAssignment a,  PnTask t, PnPerson p,  PnProjectSpace ps,  PnSpaceHasSpace shs "
				+ " WHERE a.comp_id.objectId = t.taskId  AND a.comp_id.personId = p.personId AND a.recordStatus = 'A' AND "
	...
	/* (non-Javadoc)
	 * @see net.project.hibernate.dao.IPnAssignmentDAO#getWorkSumByMonthForBusiness(java.lang.Integer, java.util.Date, java.util.Date)
	 */
	public List getWorkSumByMonthForBusiness(Integer resourceId, Integer businessId, Date startDate, Date endDate) {
		...
		if(businessId != null){
			sql +=  " AND pshp.comp_id.personId " +
					" IN ( SELECT distinct pshp1.comp_id.personId FROM PnSpaceHasPerson pshp1 " +
					" WHERE pshp1.comp_id.spaceId = :businessId ) ";
		} else {
			sql +=  " AND pshp.comp_id.personId  " +
					" IN ( SELECT distinct pshp1.comp_id.personId FROM PnSpaceHasPerson pshp1, PnBusiness b " +
					"      WHERE pshp1.comp_id.spaceId " +
					"      IN ( SELECT pshp2.comp_id.spaceId FROM PnSpaceHasPerson pshp2 "+
	                "           WHERE pshp2.comp_id.personId = :resourceId ) " +
	                "      and b.businessId = pshp1.comp_id.spaceId" +
	...
	/* (non-Javadoc)
	 * @see net.project.hibernate.dao.IPnAssignmentDAO#getResourceAssignmentSummaryByBusiness(java.lang.Integer, java.lang.Integer, java.util.Date, java.util.Date)
	 */
	public List getResourceAssignmentSummaryByBusiness(Integer resourceId, Integer businessId, Date startDate, Date endDate) {
		...
		String from = " FROM  PnTask t, PnPlanHasTask pht, PnAssignment a, PnSpaceHasPlan shp, "
				+ " PnPlan pp, PnProjectSpace ps, PnPerson p ";
		
		String where = " WHERE "
				+ " t.taskId = pht.comp_id.taskId "
				+ " AND t.taskId  = a.comp_id.objectId "
			    + " AND pht.comp_id.planId = pp.planId "
			    + " AND ps.projectId = shp.comp_id.spaceId " 
			    + " AND a.comp_id.personId = p.personId "     
			    + " AND shp.comp_id.planId = pp.planId " 
			    + " AND t.recordStatus = 'A' "
			    + " AND a.recordStatus = 'A' "
			    + " AND ps.recordStatus = 'A' "
			    + " AND p.userStatus != '"+ PersonStatus.DELETED.getID() + "'";
		
		if (businessId != null) {
			from += ", PnSpaceHasPerson bshp ";
			where = where + "  AND bshp.comp_id.spaceId = :businessId  ";
		} else {
			where +=  " AND p.personId IN " 
					  + " (SELECT  distinct pshp.comp_id.personId " 
		...
		if (startDate != null && endDate != null) {
			where += " AND ((a.startDate BETWEEN :startDate AND :endDate " 
					  + " OR a.endDate BETWEEN :startDate AND :endDate )"
					  + " OR (a.startDate <= :startDate AND a.endDate >= :endDate ))";
		}
	...
	public List<PnAssignment> getResourceAssignmentDetails(Integer resourceId, Integer[] projectIds, Date startDate, Date endDate){
		...
		String sql = "SELECT new PnAssignment (p.displayName, ps.projectId, ps.projectName, " 
			        ...
			        +" FROM PnAssignment a, PnTask t, PnObject o ,PnProjectSpace ps, PnPerson p "
					+" WHERE a.comp_id.spaceId = ps.projectId "
					+" AND a.comp_id.objectId = o.objectId "
					+" AND o.objectId = t.taskId "
					+" AND a.comp_id.personId = p.personId "
					+" AND a.comp_id.personId = :resourceId "					
					+" AND t.recordStatus = 'A' "
					+" AND a.recordStatus = 'A' " 
					+" AND ps.recordStatus = 'A' ";
	...
	public List<PnAssignment> getOverAllocatedResources(Integer userId){
		...
		String sql = " SELECT a.startDate, SUM (a.percentAllocated), "
					 +" p.personId, p.firstName, p.lastName, p.displayName "
				     + " FROM   PnAssignment a, PnTask t, PnPerson p, PnSpaceHasSpace shs "
				     + " WHERE  t.taskId = a.comp_id.objectId "
				     + " AND    p.personId = a.comp_id.personId  " 
				     + " AND    t.recordStatus = 'A' "
				     + " and    a.comp_id.spaceId = shs.comp_id.childSpaceId "
				     + " and    shs.comp_id.parentSpaceId IN "
					 + " ( SELECT b.businessId "
					 + "   FROM PnBusiness b, PnObject o, PnPerson p "
					 + "   WHERE o.objectId = b.businessId "
					 + "   AND p.personId = o.pnPerson.personId "
					 + "   AND o.recordStatus = 'A' "
					 + "   AND p.recordStatus = 'A' "
					 + "   AND o.pnPerson.personId = :userId ) "
				     + " GROUP BY p.personId, p.firstName, p.lastName, p.displayName, a.startDate " 
				     + " HAVING   SUM (a.percentAllocated) > 100 "
	...
	/* (non-Javadoc)
	 * @see net.project.hibernate.dao.IPnAssignmentDAO#getOverAssignedResources(java.lang.Integer)
	 */
	public List<PnAssignment> getOverAssignedResources(Integer userId, Date startDate, Date endDate) {
		...
		String sql = "select p.personId, p.firstName, p.lastName, p.displayName, t.taskId, t.taskName, " 
				+ " a.startDate, a.endDate, a.percentAllocated , a.work "
				+ " from PnAssignment a, PnPerson p, PnTask t, PnSpaceHasPerson shp  "
				+ "	where a.comp_id.objectId = t.taskId and a.comp_id.personId = p.personId and a.recordStatus = 'A' " 
				+ " and t.recordStatus = 'A' and p.personId = shp.comp_id.personId and "
				+ " shp.comp_id.spaceId in " 
	...
	/* (non-Javadoc)
	 * @see net.project.hibernate.dao.IPnAssignmentDAO#getCurrentOverAssignedResourcesForProject(java.lang.Integer, java.util.Date)
	 */
	...
	/* (non-Javadoc)
	 * @see net.project.hibernate.dao.IPnAssignmentDAO#getAssignmentsByPersonForProject(java.lang.Integer, java.util.Date, java.util.Date)
	 */
	public List<Teammate> getAssignmentsByPersonForProject(Integer projectId, Date startDate, Date endDate) {
		...
		String sql = "SELECT DISTINCT p.personId, p.firstName, p.lastName, p.displayName, p.email, " 
			...
			+ " a.percentComplete , t.taskId, t.taskName, ppp.skype, p.userStatus "
			+ " FROM PnAssignment a, PnTask t,"
			+ " PnPerson p LEFT JOIN p.pnPersonProfile ppp "
			+ "	WHERE a.comp_id.objectId = t.taskId AND a.comp_id.personId = p.personId " 
			+ " AND a.recordStatus = 'A' AND t.recordStatus = 'A' " 
			+ " AND p.userStatus IN ('Active', 'Unregistered') " 
	...
	public List<ReportUser> getAssignedProjectsByBusiness(String userId, Integer businessId, Date startDate, Date endDate){
		...
		String sql = "SELECT distinct p.personId , p.firstName , p.lastName, ps.projectId, ps.projectName " +
				...
				" AND ps.recordStatus='A' AND p.userStatus !='Deleted' AND a.recordStatus = 'A' " +
		...
		if(businessId != null){
			...
			sql +=  " AND pshp.comp_id.personId  " +
					...
					"      WHERE pshp1.comp_id.spaceId " +
					"      IN ( SELECT pshp2.comp_id.spaceId FROM PnSpaceHasPerson pshp2 "+
	                "           WHERE pshp2.comp_id.personId = :userId ) " +
	                "      AND b.businessId = pshp1.comp_id.spaceId AND b.recordStatus = 'A' )";			
		}		
	...
	/* (non-Javadoc)
	 * @see net.project.hibernate.dao.IPnAssignmentDAO#getTeammatesWithoutAssignments(java.lang.Integer, java.util.Date, java.util.Date)
	 */
	@SuppressWarnings("unchecked")
	public List<Teammate> getTeammatesWithoutAssignments(Integer projectId,  Date date) {
		...
		String sql = " SELECT DISTINCT new net.project.hibernate.model.project_space.Teammate(p.personId, p.firstName, " 
				   ...
				   + " FROM PnPerson p LEFT JOIN p.pnPersonProfile ppp "
				   + " WHERE p.userStatus IN ('Active', 'Unregistered') "
				   + " AND p.personId IN (SELECT shp.comp_id.personId FROM PnSpaceHasPerson shp " 
				   +		              " WHERE shp.relationshipPersonToSpace = 'member' AND shp.comp_id.spaceId = :projectId) "
				   + " AND NOT EXISTS (SELECT a.comp_id.personId " 
				   +                   " FROM PnAssignment a, PnObject o " 
				   +                   " WHERE a.comp_id.personId = p.personId AND a.recordStatus = 'A' AND " 
...
//        String sql = "SELECT a, p1.displayName, p2.displayName, on1.name, on2.name, o1.pnObjectType.objectType, o2.pnObjectType.objectType, pv.timezoneCode " +
...
//                    assignment.populate(pnAssignment, assignorName, assigneeName, objectName, objectType, spaceName, objectType, timeZoneId);
//                    assignments.add(assignment);
//                }
//
//            }
//        } catch (Exception e) {
//            log.error("Error occured while getting assignments of user from project : " + e.getMessage());
//            e.printStackTrace();
//        }
//        return assignments;
//    }
//    
    public List<PnAssignment> getAssignorAssignmentDetails(Integer assignorId){
		...
		String sql = "SELECT new PnAssignment (p.displayName, ps.projectId, ps.projectName, " 
					...
					+" AND a.comp_id.objectId = o.objectId "
					+" AND o.objectId = t.taskId "
					+" AND a.pnAssignor.personId = p.personId "
					+" AND a.pnAssignor.personId = :assignorId "					
					+" AND t.recordStatus = 'A' "
					+" AND a.recordStatus = 'A' " 
					+" AND ps.recordStatus = 'A' ";
    ...
    public List<PnAssignment> getAssignmentDetailsWithFilters(
    	...
    	String sql = "SELECT new PnAssignment (" 
					...
					+" FROM PnAssignment a left join  a.pnAssignor assignor " 
					+" left join a.pnTask task left join a.pnClassInstance ci, " 
					+" PnObject o1, PnObject o2, " 
			        +" PnObjectName on1, PnObjectName on2, " 
			        +" PnPerson p "
					+" WHERE o1.objectId = a.comp_id.spaceId "
					+" AND o2.objectId = a.comp_id.objectId "
					+" AND on1.objectId = a.comp_id.spaceId "
					+" AND on2.objectId = a.comp_id.objectId "
					+" AND a.recordStatus = 'A' " 
                    +" AND a.statusId != '" + AssignmentStatus.REJECTED.getID() + "' " // FIX FOR: bug-775
					+" AND o1.recordStatus = 'A' "
					+" AND o2.recordStatus = 'A' "
					+" AND p.personId = a.comp_id.personId ";
    	
    	
    	 if(personIds != null){
    		if(assigneeORAssignor != null && assigneeORAssignor.equalsIgnoreCase("assignor")){//assignor Assignment
				sql += " AND a.pnAssignor.personId IN (:personIds) ";
			} else if(assigneeORAssignor != null && assigneeORAssignor.equalsIgnoreCase("assignee")){//resource Assignment 
		...
		if(businessId != null){
			sql += " AND( a.comp_id.spaceId IN("
				+" select distinct shs.comp_id.childSpaceId from PnSpaceHasSpace shs " 
				+" where shs.parentSpaceType = 'business' "
				+" and	shs.comp_id.parentSpaceId = :businessId and shs.recordStatus = 'A') "
		...
		if(lateAssignment){
			sql += " AND ((a.percentComplete is null) OR " 
		            +" (a.endDate <  :toDayDate AND " 
		            +" (a.percentComplete < 1))) ";
		}
    ...
    public Integer getTotalAssignmentCountWithFilters(
    	...
    	String sql = " SELECT count(a.comp_id.objectId) "
		    		+" FROM PnAssignment a left join  a.pnAssignor assignor " 
		    		+" left join a.pnTask task, " 
					+" PnObject o1, PnObject o2, " 
				    +" PnObjectName on1, PnObjectName on2, " 
				    +" PnPerson p "
					+" WHERE o1.objectId = a.comp_id.spaceId "
	...
	public PnAssignment getAssigmentByAssignmentId(Integer objectId, Integer userId) {
		...
		try {
			String sql = 
				" FROM PnAssignment a  WHERE a.comp_id.objectId = :objectId AND( a.pnAssignor.personId = :userId OR a.comp_id.personId = :userId ) AND a.recordStatus = 'A' ";
	...
	public PnAssignment getPersonAssignmentForObject(Integer objectId, Integer personId){
		PnAssignment assignment = new PnAssignment();
		try {
			String sql = " FROM PnAssignment a  WHERE a.comp_id.objectId = :objectId "
				+ " AND a.comp_id.personId = :personId "
				+ " AND a.recordStatus = 'A' "
