...
-- Ivana       31-01-09  while copying form, set status to pending except if copy_all procedure is called
--                       The active status forms are again activated in java layer. (test2:bug104)
...
----------------------------------------------------------------------
    ...
    is
        ...
        is
            ...
            and c.record_status <> 'D' and shc.IS_OWNER = 1;
    ...
    begin
        ...
        loop
            copy_form(v_from_space_id, v_to_space_id, form_rec.class_id, form_rec.is_owner, v_created_by_id, true, v_new_form_id);
        end loop;
...
begin
    ...
    copy_form(i_from_space_id, i_from_space_id, i_from_form_id, v_is_from_space_owner,
              i_created_by_id,false, v_new_form_id);
...
----------------------------------------------------------------------
    procedure copy_form (
        ...
        i_created_by_id in number,
        b_keep_record_status in boolean,
        o_new_form_id out number)
    is
    ...
    begin
        ...
        then
            ...
            if (b_keep_record_status) then
               v_record_status := form_rec.RECORD_STATUS;
            else
               v_record_status := 'P';
            end if;
