...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="assignment_list">
        <table border="0" width="100%">
            <tr>
                <td class="tableHeader"><xsl:value-of select="display:get('prm.resource.allocationlist.dates.name')"/></td>
                <td class="tableHeader"><xsl:value-of select="display:get('prm.resource.allocationlist.spacename.name')"/></td>
                <td class="tableHeader"><xsl:value-of select="display:get('prm.resource.allocationlist.percentassigned.name')"/></td>
                <td class="tableHeader"><xsl:value-of select="display:get('prm.resource.allocationlist.taskname.name')"/></td>
            </tr>
            <tr>
