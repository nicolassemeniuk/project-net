...
|     $Author: avinash $
|
|  Creates a License from an encrypted string
|--------------------------------------------------------------------%>
<%@ page
    contentType="text/html; charset=UTF-8"
    info="License Creator"
    language="java"
...
function validateForm() {
	var errorMessage = '<%=PropertyProvider.get("prm.global.javascript.general.please.paste.licence.error")%>';
    if (!checkTextbox(theForm.enteredLicense,errorMessage)) return false;
    return true;
}
...
function reset() { self.document.location = JSPRootURL + "/admin/license/LicenseCreator.jsp?module=<%=Module.APPLICATION_SPACE%>&action=<%=Action.MODIFY%>"; }

function help() {
   	openwin_help(JSPRootURL + "/help/Help.jsp?page=admin&section=license_externalcreate");
}
    ...
    <tb:setAttribute name="leftTitle">
        <history:history>
            <history:page display="Create License"
                          jspPage='<%=SessionManager.getJSPRootURL() + "/admin/license/LicenseCreator.jsp"%>'
...
<form name="main" action="<%=SessionManager.getJSPRootURL() + "/admin/license/LicenseCreatorProcessing.jsp"%>" method="post">
...
<table width="100%" cellpadding="0" cellspacing="0" border="0">
	<tr class="channelHeader" align="left">
		...
		<th class="channelHeader">&nbsp;<%=PropertyProvider.get("prm.project.admin.license.create.label") %></th>
        ...
        <table>
            ...
            <tr>
                <td class="fieldContent" colspan="2">
                    <code>
                    <textarea name="enteredLicense" rows="25" cols="80"><%=(request.getParameter("licenseKey") != null ? request.getParameter("licenseKey") : "")%></textarea>
                    </code>
                </td>
            </tr>
        </table>
