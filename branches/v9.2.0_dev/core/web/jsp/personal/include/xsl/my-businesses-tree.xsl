...
<xsl:variable name="numCols" select="number($maxDepth + 1 + 2)" />
...
<table border="0" cellpadding="0" cellspacing="0" width="100%" name="tableWithEvenRows">
    <tr align="left" class="table-header">
        <td class="table-header" colspan="{$colspan}"><xsl:value-of select="display:get('prm.personal.main.business.label')"/></td>
        <td class="table-header"><xsl:value-of select="display:get('prm.personal.main.businesstype.label')"/></td>
        <td class="table-header"><xsl:value-of select="display:get('prm.personal.main.people.label')"/></td>
    </tr>
    <!-- tr class="tableLine">
...
<xsl:template match="BusinessSpace">
    <xsl:param name="level" />
...
<tr align="left" valign="middle" class="tableContent">
    ...
    <td colspan="{$colspan}" align="left" class="tableContent">
        <xsl:choose>
            <xsl:when test="../IsMember = 1">
                <a href="../business/Main.jsp?id={businessID}"><xsl:value-of select="name"/></a>
            </xsl:when>
            <xsl:otherwise>
    ...
    <td class="tableContent" align="left"><xsl:value-of select="businessType"/></td>
