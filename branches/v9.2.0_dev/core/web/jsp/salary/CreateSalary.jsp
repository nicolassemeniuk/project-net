...
<%
	...
	PnPersonSalary lastSalary = ServiceFactory.getInstance().getPnPersonSalaryService().getLastPersonSalaryByPersonId(Integer.valueOf(user.getID()));
	String minStartDate = SessionManager.getUser().getDateFormatter().formatDate(DateUtils.addDay(lastSalary.getStartDate(), 1));
%>
...
<script language="javascript">
	...
	var userDateFormatString = '<%=SessionManager.getUser().getDateFormatter().getDateFormatExample()%>';    
...
<body class="main" onLoad="setup();">
	...
	<form method="post" action="CreateSalaryProcessing.jsp">
		...
		<input type="text" name="startDate" id="startDate" value="<%=SessionManager.getUser().getDateFormatter().formatDate(new Date())%>" size="08" maxlength="08" class="inputSearchField">
