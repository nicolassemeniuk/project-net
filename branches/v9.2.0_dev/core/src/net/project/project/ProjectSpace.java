...
public class ProjectSpace extends Space implements IPortfolioEntry, IJDBCPersistence, IXMLPersistence, Serializable, Comparable {
    ...
    public void create(DBBean db) throws PersistenceException {
        ...
        if (this.updateWorkspaceRelationshipsOnCreate) {
            ...
            if ((this.parentBusinessID != null) && !this.parentBusinessID.equals("")) {
    			...
    			// Inheriting all business owned roles to project when project visibility is OWNING_BUSINESS_PARTICIPANTS
    ...
    public void copyDefaultForms() throws PersistenceException {
    	...
    	// after creation of project ID should not be null, otherwise it is exception condition
        if (getID() == null)
    ...
    public BusinessSpace getBusinessSpace() {
        ...
        String query = "select parent_space_id as business_id from pn_space_has_space where parent_space_id= " + getID() + " and relationship_parent_to_child = 'owns'";
    ...
    /**
	 * details of this project object in html format.
	 * By usnig a template project-detail.tml
	...
	/**
     * This method returns the project name and url as XML text for notification.
     *
	...
	/**
	 * To get Project details html presentation to be displayed in left side tool bar 
	 * @return string
	 */
	public String getProjectSpaceDetails(){
		...
		try{
			...
			detailsHtmlString+= "<br><div id=\"leftSpaceName\" style=\"margin-top: 15px;\" class=\"project-title\">" + HTMLUtils.escape(this.getName()) + "</div><br clear=\"both\"/> ";

			if(StringUtils.isNotEmpty(this.getMetaData().getProperty("ProjectManager"))){
				detailsHtmlString+= "<div class=\"project-description\"><strong>" + PropertyProvider.get("prm.project.portfolio.finder.column.meta.projectmanager") + 
									"</strong><br /> "+ HTMLUtils.escape(this.getMetaData().getProperty("ProjectManager")) + "</div>";
			}
			if(StringUtils.isNotEmpty(this.getStatus())){
				detailsHtmlString+= "<div class=\"project-description\"><strong>" +  PropertyProvider.get("prm.project.dashboard.status.label") +
									"</strong>" + this.getStatus() + "</div>";
			}
     ...
     * @param obj the ProjectSpace object
     * @return int
     */
