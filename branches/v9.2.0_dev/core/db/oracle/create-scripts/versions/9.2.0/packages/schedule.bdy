...
t_parent_task_id PN_TASK.PARENT_TASK_ID%TYPE;
...
BEGIN
    ...
    IF ((i_task_id IS NULL) OR (i_task_id = ''))    THEN
        ...
        INSERT INTO pn_task (
             ...
             calculation_type_id, unallocated_work_complete, unallocated_work_complete_unit, unassigned_work, unassigned_work_units, wbs, wbs_level
        ) VALUES (
             ...
             i_unassigned_work, i_unassigned_work_units, i_wbs, i_wbs_level
        );
    ...
    ELSE
        ...
        if (((v_prev_parent_id is null) and (i_parent_task_id is not null)) or
			...
			-- Keep task position as old parent task sequence while outdenting a task
			t_parent_task_id := i_parent_task_id;
	        if (i_unindent_status = 1) then 
	            t_parent_task_id := v_prev_parent_id;
	        end if;
            ...
            start with t.task_id = t_parent_task_id
            connect by t.parent_task_id = prior t.task_id
            order by   t.date_start, t.rowid;
...
(
    ...
    i_inline_editing_warning IN NUMBER,
    i_un_assigned_workcapture IN NUMBER,
    i_hours_per_day IN NUMBER,
    i_hours_per_week IN NUMBER,
    i_days_per_month IN NUMBER,
    i_resource_calendar IN VARCHAR2,
    ...
    if (io_plan_id is null) then
        ...
        insert into pn_plan
          (plan_id, plan_name, plan_desc, date_start, date_end,
           ...
           constraint_type_id, constraint_date, inline_editing_warning, un_assigned_workcapture, 
	   hours_per_day, hours_per_week, days_per_month, resource_calendar)
        values
	   ...
	   i_inline_editing_warning, i_un_assigned_workcapture, i_hours_per_day,
	   i_hours_per_week, i_days_per_month, i_resource_calendar);
	  ...
	  inline_editing_warning = i_inline_editing_warning,
	  un_assigned_workcapture = i_un_assigned_workcapture,
	  hours_per_day = i_hours_per_day,
	  hours_per_week = i_hours_per_week,
	  days_per_month = i_days_per_month,
	  resource_calendar = i_resource_calendar
        where
          plan_id = io_plan_id;
