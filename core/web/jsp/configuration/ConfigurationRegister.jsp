...
<%-- Import CSS --%>
<template:getSpaceCSS space="configuration" />
...
if ((request.getParameter("external") != null) && (request.getParameter("inframe") == null)) {
    session.putValue("requestedPage", request.getRequestURI() + "?id="+
                     request.getParameter("id") + "&module=" + request.getParameter("module"));
...
<%
	configurationAssignmentManager.setUser(user);
	configurationAssignmentManager.setConfigurationID(request.getParameter("id"));
	configurationAssignmentManager.load();

	// this is the only security check we need
	if (!configurationAssignmentManager.isLoaded()) {
...
<form name="main" method="post" action="ConfigurationRegisterProcessing.jsp">
...
<table width="97%" border="0" cellspacing="0" cellpadding="0">
	<tr class="channelHeader">
		...
		<td nowrap class="channelHeader" width="85%"><jsp:getProperty name="configurationAssignmentManager" property="configurationName" /></td>
	...
	<tr valign="top">
		...
		<tbody>
		<tr>
			<td align="left" class="tableHeader" width="20%"><%=PropertyProvider.get("prm.project.configuration.description.label") %></td>
			<td class="tableContent">
				<jsp:getProperty name="configurationAssignmentManager" property="configurationDescription" />
			</td>
		</tr>
		<tr>
			<td align="left" class="tableHeader" width="20%"><%=PropertyProvider.get("prm.project.admin.responsabilities.label")%></td>
			<td class="tableContent">
				<jsp:getProperty name="configurationAssignmentManager" property="responsibilities" />
			</td>
		</tr>
		</tbody>
		</table>
