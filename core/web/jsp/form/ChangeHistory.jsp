...
<html>
<head>
<title>Form Change History</title>
<link rel="stylesheet" type="text/css" href="<%= SessionManager.getJSPRootURL() %>/styles/pnet.css">
<%@ page 
...
function reset() {self.document.location = JSPRootURL + "/form/ChangeHistory.jsp"; }
</script>

</head>
<body class="main" bgcolor="#FFFFFF" onLoad="setup();">
...
<%--	  	<a href="AccessHistory.jsp?id=<%=request.getParameter("id")%>">Access History</a> 
		&nbsp;&nbsp; --%>
		<a href="<%=SessionManager.getJSPRootURL()%>/form/ChangeHistory.jsp?module=<%=net.project.base.Module.FORM%>&action=<%=net.project.security.Action.MODIFY%>&id=<%=request.getParameter("id")%>">Change History</a>
		&nbsp;&nbsp;
		<a href="<%=SessionManager.getJSPRootURL()%>/form/FormEdit.jsp?module=<%=net.project.base.Module.FORM%>&action=<%=net.project.security.Action.MODIFY%>&id=<%=request.getParameter("id")%>">Form</a>
	</td>
