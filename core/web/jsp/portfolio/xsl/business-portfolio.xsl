...
<xsl:variable name="numCols">
    <xsl:choose>
        <xsl:when test="string($radioOptionName)">9</xsl:when>
  ...
  <xsl:template match="ProjectPortfolio">
  	<table cellpadding="0" cellspacing="0" border="0" width="100%">
		<tr class="tableHeader" align="left" valign="top">
            ...
            <td class="tableHeader"><xsl:value-of select="display:get('prm.business.project.portfolio.status.column')"/></td>
            <!-- Overall Status -->
			...
			<td colspan="2" class="tableHeader"><xsl:value-of select="display:get('prm.business.project.portfolio.completionpercentage.column')"/></td>
		</tr>
		<tr class="tableLine">
	...
	<xsl:template match="ProjectSpace">
		<tr align="left" valign="middle" class="tableContent">
			...
			<td class="tableContent" width="110">
			...
			<td class="tableContent"><xsl:value-of select="format:formatPercent(percent_complete,0,0)"/></td>
         </tr>
        <tr class="tableLine">
	        <td colspan="{$numCols}">
