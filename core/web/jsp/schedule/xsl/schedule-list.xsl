...
<xsl:stylesheet version="1.0"
    ...
    extension-element-prefixes="display format personProperty" >
...
<xsl:template match="task|summary">
	...
	<tr class="tableContent {$rowClass}" id="{id}">
		<td valign="bottom" width="1%"><xsl:text disable-output-escaping="yes">&amp;nbsp;</xsl:text></td>
		<td>
		...
		<td nowrap="" width="20%">
            ...
            <xsl:choose>
                <xsl:when test="string-length(./name) > 40">
                    <a href="{$JSPRootURL}/servlet/ScheduleController/TaskView?module=60&amp;action=1&amp;id={id}"><xsl:value-of select="display:get('prm.global.textformatter.truncatedstringformat', substring(name,1,40))"/></a>
                    <xsl:text disable-output-escaping="yes">&amp;nbsp;&amp;nbsp;</xsl:text>
                </xsl:when>
                <xsl:otherwise>
                    <a href="{$JSPRootURL}/servlet/ScheduleController/TaskView?module=60&amp;action=1&amp;id={id}"><xsl:value-of select="./name"/></a>
                    <xsl:text disable-output-escaping="yes">&amp;nbsp;&amp;nbsp;</xsl:text>
                </xsl:otherwise>
            </xsl:choose>
		</td><!-- BFD 3042 //sjmittal re fixing this --> 
		<td><xsl:value-of select="format:formatNumber(work)"/><xsl:text disable-output-escaping="yes">&amp;nbsp;</xsl:text><xsl:value-of select="workUnits"/> </td>
