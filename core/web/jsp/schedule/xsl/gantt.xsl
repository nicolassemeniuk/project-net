...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:variable name="num_minor_units" select="/gantt/timescale/minor_scale/total_units"/>
    ...
    <xsl:template match="timescale/major_scale">
        <tr bgcolor="#ffffff">
            <td colspan="{$major_unit_colspan}" nowrap=""/>
            <xsl:apply-templates select="major_unit"/>
        </tr>
    </xsl:template>

    <xsl:template match="gantt">
        <xsl:call-template name="gantt_css"/>
    ...
    <xsl:template match="timescale/minor_scale">
        <xsl:param name="minor_units" select="/gantt/timescale/minor_scale/minor_unit"/>

        <xsl:variable name="scale_type" select="./resolution"/>

        <xsl:choose>
            <xsl:when test="$scale_type='day'">
    ...
    <xsl:template match="timescale">
        <xsl:apply-templates select="major_scale"/>
        <xsl:apply-templates select="minor_scale"/>
    </xsl:template>

    <xsl:template match="major_unit" name="major_unit">
        <td colspan="{$major_unit_colspan}" class="majorScale" nowrap="">
            <xsl:value-of select="."/>
        </td>
    </xsl:template>

    <xsl:template match="minor_unit" name="minor_unit">
        <xsl:variable name="unit" select="."/>

        <xsl:choose>
            <xsl:when test="contains($unit, 'S')">
                <td class="nonWorkingDay">
    ...
    <xsl:template match="tasklist" name="tasklist">
        <xsl:for-each select="/gantt/tasklist/task">
            <tr class="taskRow">
                <td colspan="{$major_unit_colspan}" class="taskHeader" nowrap="">
                    <img border="0" height="1" src="../images/spacers/trans.gif">
                        <xsl:attribute name="width"><xsl:value-of select="(number(hierarchy_level)-1)*15"/></xsl:attribute>
                    </img>
    ...
    <xsl:template match="tasklist/task" name="task">
        ...
        <!--
        Block Position: <xsl:value-of select="$block_position"/>
        Num Minor Units: <xsl:value-of select="$num_minor_units"/>
        <xsl:variable name="comparo">
            <xsl:choose>
                <xsl:when test="number($block_position) &lt; number($num_minor_units)">Less Than</xsl:when>
                <xsl:when test="number($block_position) &gt; number($num_minor_units)">Greater Than</xsl:when>
            </xsl:choose>
        </xsl:variable>
        Comparo: <xsl:value-of select="$comparo"/>
        -->
            ...
            <xsl:choose>
                ...
                <xsl:when test="$block_position = start_block">
                    <xsl:choose>
                        ...
                        <xsl:when test="type = $taskTypeSummary">
                            <td colspan="{duration_blocks}">
                                <table cellpadding="0" cellspacing="0" border="0" width="100%">
                                    <tr>
                                        <td class="summaryTask"><img src="../images/schedule/gantt2.gif" height="12" width="11" border="0"/></td>
                                        <td class="closeSummaryTask"><img src="../images/schedule/gantt2.gif" height="12" width="11" border="0"/></td>
                                    </tr>
                                </table>
                            </td>
                        </xsl:when>
    ...
    <xsl:template match="is_non_working_day" name="is_non_working_day">
        <xsl:param name="day"/>
        ...
        -->
        <xsl:value-of select="$non_working_day"/>
    </xsl:template>

    <xsl:template match="has_active_task" name="has_active_task">
        <xsl:param name="block_count"/>
        ...
        <xsl:variable name="has_active_task">
            <xsl:choose>
                <xsl:when test="(number($block) &gt;= number($starting_block)) and (number($block) &lt;= number($ending_block))">1</xsl:when>
                <xsl:otherwise>0</xsl:otherwise>
            </xsl:choose>
        </xsl:variable>

        <xsl:value-of select="$has_active_task"/>
    </xsl:template>

    <xsl:template match="draw_task" name="draw_task">
        <xsl:param name="num_blocks"/>
        ...
        <td class="activeTask" colspan="{$num_blocks}">
            <xsl:choose>
                <xsl:when test="$pct_complete_width_px > 0">
                    <img class="taskPercentComplete" height="2" width="{$pct_complete_width_px + $offset}px" src="../images/lblack.gif"/>
                </xsl:when>
                <xsl:otherwise>
    ...
    <xsl:template match="draw_empty_block" name="draw_empty_block">
        <xsl:param name="num_blocks"/>

        <td class="inactiveTask" bgcolor="#ffffff" colspan="{$num_blocks}"></td>
    </xsl:template>

    <xsl:template match="draw_completion_label" name="draw_completion_label">
        <xsl:param name="num_blocks"/>
        <xsl:param name="pct_complete"/>
    ...
    <xsl:template match="draw_milestone" name="draw_milestone">
        <xsl:param name="remaining_blocks"/>
        ...
        <xsl:choose>
            ...
            <xsl:when test="number($remaining_blocks) &gt;= 3">
                ...
                <td class="taskInfo" colspan="3">
                    ...
                    <xsl:text disable-output-escaping="yes">&amp;nbsp;</xsl:text>
                    <xsl:value-of select="format:formatISODate($milestone_date)"/>
                </td>
    ...
    <xsl:template match="legend" name="legend">
                        ...
                        <TABLE align="center" border="0" cellPadding="4" cellSpacing="1">
                            <TBODY>
                                <TR>
                                    <TD class="legend">
                                        <B>
                                            <xsl:value-of select="display:get('prm.schedule.gantt.legend.label')"/>
                                        </B>
                                    </TD>
                                    <TD class="legend">
                                        <IMG class="legend" alt="" border="0" align="textTop" src="../images/summary.gif" width="25px"/>
                                        <xsl:text disable-output-escaping="yes">&amp;nbsp;</xsl:text>
                                        <xsl:value-of select="display:get('prm.schedule.gantt.legend.tasksummary.label')"/>
                                    </TD>
                                    <TD class="legend">
                                        <IMG class="legend" alt="" border="0" align="textTop" src="../images/milestone.gif" width="12"/>
                                        <xsl:text disable-output-escaping="yes">&amp;nbsp;</xsl:text>
                                        <xsl:value-of select="display:get('prm.schedule.gantt.legend.milestone.label')"/>
                                    </TD>
                                </TR>
                            </TBODY>
                        </TABLE>
    ...
    <xsl:template match="gantt_css" name="gantt_css">
        <style>
            ...
            TD.summaryTask {
            background-image: url(/Images/schedule/gantt1.gif);
