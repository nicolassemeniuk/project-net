...
Ext.ux.maximgb.treegrid.AbstractTreeStore = function(config){
...
Ext.extend(Ext.ux.maximgb.treegrid.AbstractTreeStore, Ext.data.Store,
{
	/**
...
Ext.ux.maximgb.treegrid.GridView = Ext.extend(Ext.grid.GridView, 
{
  ...
  {
	if(record.data['rowClass']){
		return record.data['rowClass'];
	}
	return "";
  },
	...
	onClick : function(e)
	{
		...
		if (row !== false) {
			...
			if (Ext.fly(target).hasClass('ux-maximgb-treegrid-elbow-active')) {
				...
				//don't call superclass click when clicked just on icon
				return;
			}
		}
		// Breadcrumb click
...
Ext.ux.maximgb.treegrid.PagingToolbar = Ext.extend(Ext.PagingToolbar,
{
  ...
  getPageData : function()
  {
	...
	{
		//[For 'Personal Assignments Dashboard': if assignmentTreeGrid is defined ]. 
		if(typeof(assignmentTreeGrid) != 'undefined'){
