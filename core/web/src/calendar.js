...
var pathToImages = JSPRootURL+'/images/calendar/';	// Relative to your HTML file
...
function pickDate(e,inputDay)
{
	...
	if(returnFormat){
		if(returnFormat.match('dd')){
			returnFormat = returnFormat.replace('dd',day);
		} else if(returnFormat.match('d')){
			returnFormat = returnFormat.replace('d',day);
		}
		if(returnFormat.match('mm')){
			returnFormat = returnFormat.replace('mm',month);		
		} else if(returnFormat.match('m')){
			returnFormat = returnFormat.replace('m',month);		
		}		
		if(returnFormat.match('yyyy')){
			returnFormat = returnFormat.replace('yyyy',currentYear);
		} else if(returnFormat.match('yy')){			
		    returnFormat = returnFormat.replace('yy',currentYear);		    
		} else if(returnFormat.match('y')){			
...
function displayCalendar(inputField,format,buttonObj,displayTime,timeInput)
{
	...
	if(inputField.value.length==format.length){
		...
		if(yearPos == -1) yearPos = format.indexOf('yy'); 
		if(yearPos == -1) yearPos = format.indexOf('y');        
