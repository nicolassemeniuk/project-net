...
public class Roster extends PersonList implements IXMLPersistence {
    ...
    /**
     * Map pointing to the invited status for the users who are currently
     * in the Roster.
     */
     ...
     * @param space the Space that this roster is for.
     */
    ...
    /**
     * Sets the space that this Roster is for.
     * This will clear out any entries in the roster
     * @param space the space that this roster is for
     */
     ...
     * @deprecated As of 7.7.1; No replacement.  This will become private; unused outside of this class.
     */
     ...
     *
     * @param personIDList default selected option(s).  This can be a single ID
     * or a comma-delimited list of ID's.
     ...
     * member's person id's.
     *
     * @param personIDList default selected option(s).  This can be a single email address
    ...
    /**
     * Returns an HTML option element for the specified person and status
     * where that person is selected.
     * @param memberStatus the current status
     * @param person the person for whom to produce the option
     * @return the option where the value is the person ID and the display is they name, including
     * status display for Rejected or Deleted members and the option is selected
     */
     ...
     * <p/>
     * Note that Rejected or Deleted persons cause an empty string to be returned,
     * since they will not be displayed when not selected.
     * @param memberStatus the current status
     * @param person the person for whom to produce the option
     * @return the option where the value is the person ID and the display is the name
     */
    ...
    /**
     * creates an HTML option list of the rosters member's ID
     * values are roster member's person id's
     * @param list <code>List</code> selected options
     * @return the HTML option list
     */
     ...
     * @param personID the database ID of the person to return.
     ...
     * <p/>
     * This is useful for locating Persons who once belonged in the Roster since their
     * IDs may still be in use in the workspace for which this Roster is loaded.
     * <p/>
     * This method maintains a cache of the Persons to make repeated requests for a person less costly.
     * @param personID the id of the person we are trying to look up
     * @return the person for that id, or null if the person does not exist in the entire application
     */
     ...
     * <p/>
     * @param person the person to locate; when null or when their ID is null,
    ...
    /**
     * Returns information about when the a person last visited a space.
     * The result may be a status or an empty string.
     * <p/>
     * <li>If the user person is not active as determined by
     * <code>!{@link net.project.resource.Person#ACTIVE_USER_STATUS}</code>
     * then their status is returned.  This might be <b>Unregistered</b> or
     * <b>Unconfirmed</b><li>
     * <li>Otherwise, the person is active and the return value is based on their
     * invitation as given by the constants in
     * {@link net.project.resource.SpaceInvitationManager}</li>
     * <li>If no status could be determined then the empty string is returned.
     * This most commonly occurs with space creators, who are active, but
     * never received an invitation</li>
     * @param person the person for whom to get the last visit information
     ...
     * Roster's project.
     * 
     * @param personId the person identification to search into this roster
     ...
     * @param person the person for whom to get the last visit date time
     * @param spaceID the ID of the space in which to look
     * @return the date and time of their last visit or null if they have
     * never visited
     */
     ...
     * space.
     * @param person the person for whom to get the last visit datetime
     * @param spaceID the id of the space in question
     * @return the datetime of their last visit as a string, or an empty string
     * if they have never visited the space (regardless of whether they are
     * regisitered, confirmed, invited etc.)
     ...
     * @param personList the list of persons to remove
     * @deprecated As of 7.7.1; Use {@link #removePerson(String)} for each person's ID instead
     */
     ...
     * <p>
     * Sends a notification to the person if the person being removed
     * is not a space administrator; only sends the notification if they
     * are not deleted nor unregistered.
     * </p>
     * @param personID the database ID of the Person to remove.
     * @throws PersistenceException if there is a problem locating the
     * space admin group or the person being removed is the last member
     * of the space admin group
     */
        ...
        try {
            ...
            if (!spaceAdmin.isMember(personID)) {
                ...
                if (!(person.hasStatus(PersonStatus.DELETED) || person.hasStatus(PersonStatus.UNREGISTERED))) {
                    // Only send the delete notification if the person is neither
                ...
                if (spaceAdmin.isMember(personID) &&
                        spaceAdmin.getPersonMembers().size() == 1) {
     ...
     *
     * @param person a <code>Person</code> object containing information that
     * needs to be saved.
     */
    ...
    /**
     * Reload the current roster list with existing search, sort and filter criteria.
     */
    public void reload() {
    ...
    /**
     * Searches the database for people who's first name or last name match the passed string
     * in the current space.
     * <p/>
     * The specified key is saved so that it is available via {@link #getSearchKey()}.
     * @param key the text fragment to search the people's first and last names for.
     * @throws NullPointerException if the current space or its ID is null
     */
    public void search(String key) {
        ...
        if ((key != null) && (key.length() > 0)) {
            // if the key is a single character, we will do an search for all first names beginning with that character
            if (key.length() == 1) {
                key = key + "%";
        ...
        // even if no rows are found by query, we still set isLoaded= true (0 results for search may be expected);
        setIsLoaded(true);
    }
...
//                "order by lower(p.first_name) asc, lower(p.last_name) asc ";
        ...
        final String query = "select " +
						 ...
						 " from "+
						     ...
						     " pn_address a "+
						" where "+
						     "  shp.space_id = " + getSpaceID() + 
						     "  and p.person_id = shp.person_id "+
						     "  and p.record_status = 'A' "+
						     "  and u.user_id(+) = p.person_id "+
						     "  and a.address_id(+) = pp.address_id "+
    ...
    /**
     * Populates the specified person with the column values appropriate for a person
     * and the subset of Address fields appropriate for displaying in the roster.
     * @param person the person to populate
     * @param result the resultset from which to get the column values
    ...
    protected void populate(Person person, ResultSet result) throws SQLException {
        ...
        // this is just enough address info for display in the roster, not fully loaded.
    ...
    public class InvitationStatusChecker {
         ...
         * <p/>
         * These attributes are the only ones required for displaying in the roster
         * @param person the person for whom to get the status, and to update
         * @return the status
         */
   ...
   public static String getXMLMailBody(String displayName) {
		StringBuffer sb = new StringBuffer();
    	String directoryUrl = SessionManager.getJSPRootURL() + "/" + Space.PROJECT_SPACE + "/DirectorySetup.jsp?module="
    						  + Module.DIRECTORY + "&id=" + SessionManager.getUser().getCurrentSpace().getID();
