...
public class BusinessSpaceFinder {
	...
	/**
	 * The columns to select when loading a BusinessSpace. Business table/view
	 * should be aliased as "b"
	 */
	private static final String BUSINESS_SPACE_SELECT_COLUMNS = "b.business_space_id, b.business_id, b.space_type, "
			+ "b.complete_portfolio_id, b.record_status, b.is_master, " + "b.business_category_id, b.brand_id, b.billing_account_id, b.address_id, "
			+ "b.business_name, b.business_desc, b.business_type, b.logo_image_id, " + "b.is_local, b.remote_host_id, b.remote_business_id, b.num_projects, "
			+ "b.num_people ";
	 ...
	 *             if there is a problem loading the BusinessSpace
	...
	/**
	 * Finds and loads a business space for the specified id. Populates the
	 * specified business space.
	 * 
	 * @param businessSpaceID
	 *            the id of the business space to load
	 * @param businessSpace
	 *            the business space to populate
	 * @return true if the business space was found; false otherwise
	 * @throws PersistenceException
	 *             if there is a problem loading the BusinessSpace
	 * @throws NullPointerException
	 *             if the specified businessSpaceID or businessSpace is null
	 */
	 ...
	 * 
	 * @param businessSpaceIDCollection
	 *            the colllection of <code>String</code> ids of the business
	 *            spaces to find
	 ...
	 *         represents a business ID. If any IDs are not found, no exception
	 *         is thrown; rather, the returned list will have fewer entries.
	 * @throws PersistenceException
	 *             if there is a problem loading the BusinessSpaces
	 * @throws NullPointerException
	 *             if the specified businessSpaceIDCollection is null or empty
	 */
	 ...
	 *            spaces to find
	 * @param recordStatus
	 *            the record status of businesses to find; if null, no filter on
	 *            record status will be applied
	 * @return a List where each element is a <code>BusinessSpace</code>. The
	 *         number of entries in the list will match the number of entries in
	 *         the specified collection, assuming each ID in the collection
	 *         represents a business ID and has a matching record status. If any
	 ...
	 * 
	 * @param recordStatus
	 *            the record status of BusinessSpaces to find
	 * @return a list where each element is a <code>BusinessSpace</code>; the
	...
	/**
	 * Finds all <code>BusinessSpace</code>s in the database except the space
	 * with the specified id.
	 * 
	 * @param excludedBusinessSpaceID
	 *            the id of the business space to exclude from the results
	 * @param recordStatus
	 *            the record status of BusinessSpaces to find
	 * @return a list where each element is a <code>BusinessSpace</code>; the
	 *         list is currently orderd by name, ordering performed by database
	 ...
	 *             if there is a problem loading the BusinessSpaces
	 * @throws NullPointerException
	 *             if the id or record status is null
	 */
	public final List findAllExceptOne(String excludedBusinessSpaceID, String recordStatus) throws PersistenceException {
	...
	/**
	 * Finds all <code>BusinessSpace</code>s in the database based on part of a
	 * business name.
	 * 
	 * @param recordStatus
	 *            the record status of BusinessSpaces to find
	 * @param nameFilter
	 *            part of the business name; A case insensitive search is
	 *            performed.
	 * @return a list where each element is a <code>BusinessSpace</code>; the
	 *         list is currently orderd by name, ordering performed by database
	 * @throws PersistenceException
	...
	/**
	 * Finds all <code>BusinessSpace</code>s that the specified user is a
	 * member.
	 * 
	 * @param user
	 *            the user who is a member of the business spaces
	 * @param recordStatus
	 *            the record status of BusinessSpaces to find
	 * @return a list where each element is a <code>BusinessSpace</code>; the
	 *         list is currently orderd by name, ordering performed by database
	 ...
	 *             if there is a problem loading the BusinessSpaces
	 * @throws NullPointerException
	 *             if the user or record status is null
	 */
	 ...
	 * @throws SQLException
	 *             if there is a problem accessign the database
	 */
	...
	/**
	 * Populates the specified business space from a result set row. Assumes
	...
	private static void populate(ResultSet result, BusinessSpace businessSpace) throws SQLException {
		...
		// TODO: this IF statement should be erased, all business spaces must
		// have a financial associated. Only for dev purposes for old not
		// related businesses.
		PnSpaceHasSpace relationship = ServiceFactory.getInstance().getPnSpaceHasSpaceService().getFinancialRelatedSpace(businessSpace.getID());

		if (relationship != null) {
			Integer relatedSpace = relationship.getComp_id().getChildSpaceId();
			businessSpace.setRelatedSpaceID(String.valueOf(relatedSpace));
		}
		// **********************************************************************************
