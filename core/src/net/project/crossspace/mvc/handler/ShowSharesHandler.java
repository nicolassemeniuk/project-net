...
import net.project.crossspace.ExportListGenerator;
import net.project.security.AuthorizationFailedException;
import net.project.security.SessionManager;
import net.project.space.Space;

/**
 * Generate the prerequisite information for shoing the ShowShares.jsp page.
 * That page shows a list of all things being shared from the current space.
 *
 * @author Matthew Flower
 * @since Version 8.0.0
 */
public class ShowSharesHandler extends Handler {
    public ShowSharesHandler(HttpServletRequest request) {
        super(request);
    }
    ...
    public String getViewName() {
        return "/crossspace/ShowShares.jsp";
    }
    ...
    public Map handleRequest(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        //Add the space that this information is in as verification to the user
        Space space = SessionManager.getUser().getCurrentSpace();
        model.put("space", space);

        //Get the objects exported by the current space
        ExportListGenerator listGenerator = new ExportListGenerator();
        listGenerator.setSpaceID(space.getID());
        listGenerator.load();
        model.put("xmlProvider", listGenerator);      
        passThru(model, "referer");
