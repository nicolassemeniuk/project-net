...
import net.project.crossspace.ObjectListGenerator;
import net.project.security.Action;
import net.project.security.AuthorizationFailedException;
import net.project.security.User;

public class AddExternalHandler extends Handler {
    public AddExternalHandler(HttpServletRequest request) {
        super(request);
    }
    ...
    public String getViewName() {
        return "/crossspace/AddExternal.jsp";
    }
    ...
    public void validateSecurity(int module, int action, String objectID, HttpServletRequest request) throws AuthorizationFailedException, PnetException {
        if (action != Action.CREATE) {
            throw new AuthorizationFailedException("Action must be CREATE");
        }
    }
    ...
    public Map handleRequest(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        ObjectListGenerator gol = new ObjectListGenerator();
        gol.loadObjectList(user.getID(), user.getCurrentSpace().getID());
        model.put("objectListGenerator", gol);

        passThru(model, "module");
        passThru(model, "action");
        passThru(model, "processing");
