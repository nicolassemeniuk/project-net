...
/**
 * MVC handler which performs all the necessary prerequisites to loading the
 * CreateShare.jsp page.
 *
 * @author Matthew Flower
 * @since Version 8.0.0
 */
public class CreateShareHandler extends Handler {
    public CreateShareHandler(HttpServletRequest request) {
        super(request);
    }
    ...
    public String getViewName() {
        return "/crossspace/CreateShare.jsp";
    }
    ...
    public Map handleRequest(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        passThru(model, "objectsToShare");
        passThru(model, "hierarchy");
        passThru(model, "referrer");

        String[] idList = (String[])request.getAttribute("idList");
        model.put("idList", DatabaseUtils.collectionToCSV(Arrays.asList(idList)));

        model.put("isContainer", new Boolean(true));
