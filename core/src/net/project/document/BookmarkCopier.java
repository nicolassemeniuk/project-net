...
/**
 * The BookmarkCopier copies {@link Bookmark} objects
 */
public class BookmarkCopier implements ICopier {
    
    private static final String BOOKMARK_COPY_SQL = "{call document.copy_bookmark(?, ?, ?, ?, ?)}";
    ...
    public String copy(net.project.database.DBBean db, String sourceObjectID, String targetContainerID, net.project.security.User currentUser) throws PersistenceException, CopyException {
    	...
    	Logger.getLogger(BookmarkCopier.class).debug("BookmarkCopier.copy(): Copying bookmark with id: " + sourceObjectID + 
        ...
        try {
            ...
            db.prepareCall(BOOKMARK_COPY_SQL);
        	...
        	Logger.getLogger(BookmarkCopier.class).error("BookmarkCopier.copy threw an SQLException: " + sqle);
            throw new PersistenceException("Error copying bookmark with id: " + sourceObjectID, sqle);
