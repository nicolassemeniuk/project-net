...
/**
 * This class is used for searching Document Objects.  It provides the forms for simple
 * and advanced searching as well as a properly formatted XML string of the results.
 *
 * @author BrianConneen
 * @since 03/00
 */
public class DocumentSearch extends GenericSearch {
    /**
     * Creates a new instance of <code>DocumentSearch</code>.
     */
    ...
    public String getDisplayName() {
        return PropertyProvider.get("prm.global.search.document.results.channel.title"); // Document
    }
    ...
    public String getSimpleSearchForm(String formName, HttpServletRequest request) {
        ...
        formString.append("\n<td class=\"channelHeader\" width=98% colspan=2 align=left NOWRAP>" + PropertyProvider.get("prm.global.search.document.simple.title") + "</td>"); // Simple Document Search:
        ...
        formString.append("\n</tr>");
        formString.append("\n<tr><td>&nbsp;</td>");
        formString.append("\n<TD class=\"tableHeader\">" + PropertyProvider.get("prm.global.search.document.simple.namedescription.label") + "&nbsp;&nbsp;</TD>"); // Name or Description:
    ...
    public String getAdvancedSearchForm(String formName, HttpServletRequest request) {
        ...
        formString.append("\n<tr>");
        formString.append("\n<td class=\"channelHeader\" width=1%><img src=\""+ SessionManager.getJSPRootURL() +"/images/icons/channelbar-left_end.gif\" width=8 height=15 alt=\"\" border=\"0\"></td>");
        formString.append("\n<td class=\"channelHeader\" width=98% colspan=2 align=left NOWRAP>" + PropertyProvider.get("prm.global.search.document.advanced.title") + "</td>"); // Advanced Document Search:
        formString.append("\n<td class=\"channelHeader\" width=1%><img src=\""+ SessionManager.getJSPRootURL() +"/images/icons/channelbar-right_end.gif\" width=8 height=15 alt=\"\" border=\"0\"></td>");
        formString.append("\n</tr>");
        formString.append("\n<tr><td>&nbsp;</td>");
        formString.append("\n<TD class=\"tableHeader\">" + PropertyProvider.get("prm.global.search.document.advanced.name.label") + "&nbsp;&nbsp;</TD>"); // Name:
        formString.append("\n<TD class=\"tableContent\">");
        formString.append("<INPUT TYPE=\"TEXT\" NAME=\"NAME\">");
        formString.append("</TD><td>&nbsp;</td></TR>");
        formString.append("\n<tr><td>&nbsp;</td>");
        formString.append("\n<TD class=\"tableHeader\">" + PropertyProvider.get("prm.global.search.document.advanced.description.label") + "&nbsp;&nbsp;</TD>"); // Description:
        formString.append("\n<TD class=\"tableContent\">");
    ...
    public void doKeywordSearch(String m_keyword) {
        ...
        query.append("SELECT doc.doc_id as id, doc.doc_name as name, doc.doc_description as description, '" + ObjectType.DOCUMENT + "' as object_type ");
        query.append(" FROM pn_doc_by_space_view doc, pn_space_has_doc_space ");
        query.append(" WHERE pn_space_has_doc_space.space_id = ");
        query.append(getFirstSpaceID() + " ");
        query.append(" AND pn_space_has_doc_space.doc_space_id = doc.doc_space_id  and doc.is_hidden ='0' ");

        if (keyword != null && keyword.length() > 0) {
            query.append(" AND (UPPER(doc.doc_name) LIKE UPPER('%");
            query.append(keyword);
            query.append("%') ");
            query.append(" OR UPPER(doc.doc_description) LIKE UPPER('%");
            query.append(keyword);
            query.append("%')) ");
        }
        query.append(" AND doc.record_status = 'A'");

        query.append(" UNION select b.bookmark_id as id, b.name, b.description, '" + ObjectType.BOOKMARK + "' as object_type ");
        query.append(" from pn_bookmark_view b where b.owner_space_id = " + getFirstSpaceID());

        if (keyword != null && keyword.length() > 0) {
            query.append(" AND (UPPER(b.name) LIKE UPPER('%");
            query.append(keyword);
            query.append("%') ");
            query.append(" OR UPPER(b.description) LIKE UPPER('%");
            query.append(keyword);
            query.append("%')) ");
        }
        query.append(" AND b.record_status = 'A'");
    ...
    public void doAdvancedSearch(HttpServletRequest m_request) {
        ...
        if (desc != null && desc.length() > 0) {
            query.append(" AND UPPER(doc.doc_description) LIKE UPPER('%");
            query.append(desc);
            query.append("%') ");
        }
        ...
        if (desc != null && desc.length() > 0) {
            query.append(" AND UPPER(b.description) LIKE UPPER('%");
            query.append(desc);
            query.append("%') ");
        }
    ...
    public String getXMLResults() {
        ...
        sb.append("<channel>\n");
        sb.append("<table_def>\n");
        sb.append("<col>" + PropertyProvider.get("prm.global.search.document.results.name.column") + "</col>\n"); // Document Name
        sb.append("<col>" + PropertyProvider.get("prm.global.search.document.results.description.column") + "</col>\n"); // Document Description
        sb.append("</table_def>\n");
        sb.append("<content>\n");
        ...
        for (int i = 0; i < m_results.size(); i++) {
            ...
            sb.append("</data_href>\n");
            sb.append("<data>" + XMLUtils.escape(description) + "</data>");
            sb.append("</row>\n");
