...
public class ProfileServiceImpl implements IProfileService {
	...
	public Integer createPersonStub(String email, String firstName, String lastName, String displayName, String userStatus) {
		Integer personObjectId = new Integer(0);
		try {
			// get PnObject service
			IPnObjectService objectService = ServiceFactory.getInstance().getPnObjectService();
			// create object for person
			PnObject personObject = new PnObject("person", new Integer(1), new Date(System.currentTimeMillis()), "A");
			// save person object in database
			personObjectId = objectService.saveObject(personObject);
			// create object for portfolio
			PnObject portfolioObject = new PnObject("portfolio", new Integer(1), new Date(System.currentTimeMillis()), "A");
			// save portfolio object into database
			Integer portfolioObjectId = objectService.saveObject(portfolioObject);
	...
	public void createPersonProfile(Integer personId, String firstName, String lastName, String displayName, String username, String email, String alternateEmail1,
		...
		try {
			...
			PnObject addressObject = new PnObject("address", new Integer(1), new Date(System.currentTimeMillis()), "A");
			Integer addressObjectId = objectService.saveObject(addressObject);
			...
			PnAddress address = new PnAddress(addressObjectId, address1, address2, address3, address4, address5, address6, address7, city, cityDistrict, region,
					stateProvence, countryCode, zipcode, officePhone, faxPhone, "A");
			...
			Integer spaceAdminGroupId = securityService.createSpaceAdminGroup(personId,personId, "Personal Space Admin");
			...
			Integer documentObjectId = documentService.createDocumentSpace(personId, personId);
			// create doc_space object
			Integer docSpaceObject = objectService.saveObject(new PnObject("doc_container", new Integer(1), new Date(System.currentTimeMillis()), "A"));
			...
			docContainerService.saveDocContainer(new PnDocContainer(docSpaceObject, "@prm.document.container.topfolder.name", "Top level document folder",  new Date(System.currentTimeMillis()), new Integer(1), 0,  new Date(System.currentTimeMillis()), "A"));
			// get Security service
			securityService.createSecurityPermissions(docSpaceObject, "doc_container", personId, new Integer(1));
			...
			Integer calendarObjectId = objectService.saveObject(new PnObject("calendar", new Integer(1), new Date(System.currentTimeMillis()), "A"));
			...
			securityService.createSecurityPermissions(calendarObjectId, "calendar", personId, new Integer(1));
