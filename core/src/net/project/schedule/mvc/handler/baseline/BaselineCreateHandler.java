...
/**
 * All the necessary prerequisites for displaying a baseline creation page.
 *
 * @author Matthew Flower
 * @since Version 7.7.0
 */
public class BaselineCreateHandler extends AbstractBaselineModifyHandler {
    public BaselineCreateHandler(HttpServletRequest request) {
        super(request);
    }
    ...
    public Map handleRequest(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        //passthrough security settings that will be used when submitting to processing
        passThru(model, "action");

        Baseline currentBaseline = getBaseline(request, false);
        model.put("baseline", currentBaseline);

        String reloadURL = SessionManager.getJSPRootURL()+"/servlet/ScheduleController/Baseline/Create";
        model.put("reloadURL", reloadURL);
        String reloadParams = "module="+Module.SCHEDULE+"&action="+Action.CREATE;
        model.put("reloadParams", reloadParams);
