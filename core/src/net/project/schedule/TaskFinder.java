...
public class TaskFinder extends Finder {
	...
	public static final ColumnDefinition WORK_COMPLETE_UNITS_COLUMN = new ColumnDefinition("t.work_complete_units",
			"prm.schedule.task.columndefs.workcompleteunits");
	...
	public static final ColumnDefinition WORK_PERCENT_COMPLETE_COLUMN = new ColumnDefinition("t.work_percent_complete",
			"prm.schedule.task.columndefs.workpercentcomplete");
	 ...
	 */
	private String baseSQLStatement = "select " + "  t.task_name, t.task_desc, t.task_type, t.priority, "
			+ "  t.duration, t.duration_units, t.work, t.work_units, t.work_complete, " + "  t.work_complete_units, t.date_start, t.date_finish, t.task_id, "
			+ "  t.actual_start, t.actual_finish, t.percent_complete, " + "  t.date_created, t.date_modified, t.modified_by, "
			+ "  t.parent_task_id, t.record_status, t.critical_path, " + "  pt.task_name as parent_task_name, t.constraint_type, "
			+ "  t.constraint_date, t.deadline, t.seq, pht.plan_id, " + "  t.ignore_times_for_dates, t.early_start, t.late_start, "
			+ "  t.early_finish, t.late_finish, ph.phase_id, ph.phase_name, ph.sequence," + "  t.work_percent_complete, t.is_milestone, shp.space_id,"
			+ "  shrd.exported_object_id, shrd.read_only, shbl.space_id as exporting_space_id, "
			+ "  spon.name as exporting_space_name, ctv.work, ctv.work_units, " + "  ctv.duration, ctv.duration_units, ctv.date_start, ctv.date_finish,"
			+ "  ctv.baseline_id, t.calculation_type_id, t.unallocated_work_complete,"
			+ "  t.unallocated_work_complete_unit, t.unassigned_work, t.unassigned_work_units, t.wbs, t.wbs_level," + "  cc.code_name " + "from "
			+ "  pn_space_has_plan shp, " + "  pn_plan p, " + "  pn_plan_has_task pht, " + "  pn_task t, " + "  pn_task pt, " + "  pn_phase_has_task phht, "
			+ "  pn_phase ph," + "  pn_shared shrd, " + "  pn_shareable shbl, " + "  pn_object_name spon, " + "  pn_current_task_version ctv, "
			+ "  pn_charge_code cc, " + "  pn_object_has_charge_code ohcc " + "where " + "  shp.plan_id = p.plan_id " + "  and shp.plan_id = pht.plan_id "
			+ "  and p.plan_id = pht.plan_id " + "  and pht.task_id = t.task_id " + "  and t.parent_task_id = pt.task_id(+) "
			+ "  and t.task_id = phht.task_id(+) " + "  and phht.phase_id = ph.phase_id(+) " + "  and ph.record_status(+) = 'A' "
			+ "  and t.task_id = shrd.imported_object_id(+) " + "  and shrd.exported_object_id = shbl.object_id(+) "
			+ "  and shbl.space_id = spon.object_id(+) " + "  and t.task_id = ctv.task_id(+) " + "  and cc.code_id (+) = ohcc.code_id "
	...
	/**
	 * This method will cause the following finder to preload task dependencies
	 * into the task. Normally task dependencies are lazy loaded, but sometimes
	 * lazy loading causes trouble, for example when large list of tasks are
	 * shown.
	 * 
	 * @param spaceID
	 *            a <code>String</code> containing the primary key of a space.
	 *            All task dependencies from this space are going to be loaded
	 *            into the cache.
	 */
	 ...
	 * the children of the summary tasks. This will only work properly if all of
	 * the schedule entries in a space were loaded.
	 * 
	 * @param scheduleEntries
	 *            a <code>List</code> of scheduleEntries which should represent
	 *            all of the schedule entries in this space. We will populate
	 *            the "children" list of all Summary Tasks in this list.
	 */
	...
	/**
	 * Given a Map of schedule entry id to schedule entries, populate all of the
	 * children of the summary tasks. This will only work properly if all of the
	 * schedule entries in a space were loaded.
	 * 
	 * @param taskMap
	 *            a <code>List</code> of scheduleEntries which should represent
	 *            all of the schedule entries in this space. We will populate
	...
	public static void populateParents(Map taskMap) {
		for (Iterator it = taskMap.values().iterator(); it.hasNext();) {
			...
			if (child instanceof SummaryTask) {
				...
				// method it will try to reload the children which is
				// specifically
			...
			if (child.hasParent()) {
				...
				// It is unlikely that the parent task is null, but a customer
				// has
	 ...
	 * 
	 * @param spaceID
	 *            a <code>String</code> value which contains the id of a space
	 *            which contains a schedule in which we are looking for late
	 *            tasks.
	 */
	 ...
	 * 
	 * @param spaceID
	 * @return a <code>List</code> object containing one or more tasks that are
	 *         coming due in the future.
	 * @throws PersistenceException
	 *             if an error occurs while loading the tasks.
	 */
	 ...
	 * 
	 * @param id
	 *            a <code>String</code> value which is the primary key of the
	 *            task you wish to load.
	 * @return a <code>List</code> of <code>Task</code> objects that were loaded
	 *         in response to that id. (There had better only be one or zero
	 *         items in the list, otherwise, we have problems.)
	 * @throws PersistenceException
	 *             if there is a problem loading tasks from the database. This
	 *             should mostly be of the unexpected error ilk, this shouldn't
	 *             normally happen.
	 */
	 ...
	 *            a <code>String</code> value which is the primary key of the
	 *            task you wish to load.
	 * @param isLoadPredecessors
	 *            true if predecessors should be loaded
	 * @param isLoadAssignments
	 *            true if assignments should be loaded
	 * @return a <code>List</code> of <code>Task</code> objects that were loaded
	 *         in response to that id. (There had better only be one or zero
	 *         items in the list, otherwise, we have problems.)
	 * @throws PersistenceException
	 ...
	 *             normally happen.
	 */
	public ScheduleEntry findObjectByID(String id, boolean isLoadPredecessors, boolean isLoadAssignments, boolean isLoadMaterialAssignments)
			throws PersistenceException {
	...
	/**
	 * Asserts that there is at most one result in the given list and returns
	 * it.
	 * 
	 * @param results
	 *            the results to check
	 * @param criteria
	 *            the criteria of the item
	 ...
	 * 
	 * @param idList
	 *            a <code>List</code> object containing one or more strings
	 *            containing task ids.
	...
	public List findByIDList(List idList) throws PersistenceException {
		addWhereClause(TaskFinder.TASK_ID_COLUMN.getColumnName() + " in (" + DatabaseUtils.collectionToCSV(idList) + ")");
		return loadFromDB();
	}
	 ...
	 * 
	 * @param planID
	 *            a <code>String</code> containing the plan id we are searching
	 *            for.
	 * @param sequenceNumber
	 *            a <code>int</code> containing the sequence number we are
	 *            searching for.
	 * @return a <code>List</code> with zero or more tasks.
	 * @throws PersistenceException
	 *             if there is an error loading the task.
	 */
	 ...
	 * 
	 * @param spaceID
	 *            a <code>String</code> value in which we are going to look for
	 *            tasks.
	 * @return a <code>List</code> of <code>Task</code> objects that were loaded
	 ...
	 * 
	 * @param id
	 *            a <code>String</code> value containing the primary key of the
	 *            task id we want to load from the database.
	 * @param taskToPopulate
	 *            a <code>Task</code> object that will be populated with data if
	 *            the task if found.
	 */
	protected boolean findByID(String id, ScheduleEntry taskToPopulate, boolean loadActiveTasksOnly, boolean loadAssignments, boolean loadMaterialAssignments)
			throws PersistenceException {
	 ...
	 * 
	 * @param spaceID
	 *            the space for which to find schedule entries
	 * @param filterActiveItemsOnly
	 *            when true, cancelled and completed schedule entries are
	 *            excluded. When false, no entries are excluded. When this
	 *            parameter is specified, <code>filterStatusID</code> is
	 *            typically specified as <code>null</code>
	 * @param hierarchyView
	 *            currently has no effect
	 * @param maximumEntries
	 *            the maximum number of entries to return
	 * @param isLoadDependencies
	 *            true if dependencies should be loaded with each schedule
	 *            entry; false if no dependencies are to be loaded. Loading
	 *            dependencies requires additional database round trips.
	 * @param isLoadAssignments
	 *            true if assignments should be loaded with each schedule entry;
	 *            false if no assignments are to be loaded. Loading assignments
	 *            requires additional database round trips.
	 * @param db
	 *            a <code>DBBean</code> which will be used to load the tasks.
	 * @return the list where each element is a <code>Task</code>.
	 * @throws SQLException
	 *             if there is a problem loading the schedule entries.
	 */
	public List findScheduleEntries(String spaceID, boolean filterActiveItemsOnly, int hierarchyView, int maximumEntries, boolean isLoadDependencies,
			boolean isLoadAssignments, boolean isLoadMaterialAssignments, DBBean db) throws SQLException {
	 ...
	 *         objects.
	 * @throws PersistenceException
	 *             when a database error occurs while attempting to load tasks.
	 */
	 ...
	 * 
	 * @param result
	 *            a <code>ResultSet</code> object which is currently pointing to
	 *            a record in a database result set that contains information
	 *            about a task. The result set point will not be modified in any
	 *            way, only values will be extracted.
	 * @param task
	 *            a task object to fill with data.
	 * @throws SQLException
	 *             when an error occurs while trying to populate
	 ...
	 * example, a task finder would populate a {@link Task} object. Any class
	 * that extends the finder base class needs to implement this method the
	 * finder can use its build-in loadFromDB method to load objects.
	 * 
	 * @param databaseResults
	 *            a <code>ResultSet</code> that provides the data necessary to
	 *            populate the domain object.
	 * @return a <code>Object</code> subclass specific to your finder that has
	 *         been populated with data.
	 * @throws SQLException
	 *             if an error occurs populating the object.
	 */
	 ...
	 * 
	 * @param taskID
	 *            a <code>String</code> value containing the primary key of a
	 *            task we are going to load.
	...
	/**
	 * Find all tasks that start after a task with a given task id. This is used
	 * to populate the task with successor while loading.
	 * 
	 * @param taskID
	 *            a <code>String</code> containing the task id for which we are
	...
	/**
	 * Returns a collection of <code>ScheduleEntryAssignment</code>s where each
	 * assignment belongs to the task with the specified ID.
	 * 
	 * @param taskID
	 *            the ID of the task for which assignments are required
	 * @return a collection where each element is a
	 *         <code>ScheduleEntryAssignment</code>; each assignment's objectID
	 *         is equal to the specified taskID
	 */
