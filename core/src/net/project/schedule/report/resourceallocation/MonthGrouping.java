...
/**
 * {@link net.project.base.finder.FinderGrouping} subclass which groups all
 * resource allocations by month.
 *
 * @author Matthew Flower
 * @since Version 7.4
 */
class MonthGrouping extends FinderGrouping {
    /**
     * Public constructor.
     *
     ...
     * groupings.
     */
    public MonthGrouping(String id, boolean isDefaultGrouping) {
        super(id,
            PropertyProvider.get("prm.schedule.report.resourceallocation.monthgrouping.name"),
            isDefaultGrouping);
    }
    ...
    public String getGroupName(Object currentObject) throws PersistenceException {
        ...
        if (groupingDate != null) {
            ...
            groupingDateFormatted = df.formatDate(rar.getReportingPeriodStart(),
                PropertyProvider.get("prm.schedule.report.resourceallocation.monthgrouping.groupheader.format"));
        } else {
