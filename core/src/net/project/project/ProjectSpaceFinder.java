...
public class ProjectSpaceFinder extends Finder {
	...
	/**
	 * Returns the base SQL statement used for all find operations. Includes
	 * <code>SELECT</code>, <code>FROM</code> and <code>WHERE</code> sections.
	 * The Where part contains clauses necessary for joining the base tables.
	 * 
	...
	/**
	 * Returns a populated <code>ProjectSpace</code> object from the current row
	 * in the specified result set.
	 * 
	 * @param databaseResults
	 *            the result set from which to populate the object; assumes this
	 *            points to a current row
	 * @return the populated ProjectSpace object
	 * @throws SQLException
	 *             if there is a problem populating the object
	 */
	protected ProjectSpace createObjectForResultSetRow(ResultSet databaseResults) throws SQLException {
	 ...
	 */
	public final List<ProjectSpace> find() throws PersistenceException {
		return loadFromDB();
	}
	 ...
	 */
	public final List<ProjectSpace> findByPortfolioID(String portfolioID) throws PersistenceException {
	 ...
	 * them to projects in the specified portfolio id
	 * 
	 * @param projectMembersOnly
	 *            , true if the projects with visibility project members only
	 *            otherwise false.
	 * @return a List where each element is a <code>ProjectSpace</code>
	 * @throws PersistenceException
	 *             if there is a problem loading the ProjectSpaces
	 ...
	 *             if the portfolioID is null
	 */
	public final List<ProjectSpace> findByPortfolioID(String portfolioID, boolean projectMembersOnly) throws PersistenceException {
		...
		// Add additional where clause to limit ProjectSpaces to those
		// for the specified portfolioID
		addWhereClause(" p.project_id in (" + " select pv.project_id " + " from pn_portfolio_view pv ,pn_project_space shpr "
				+ " where pv.project_id(+) = shpr.project_id and shpr.visibility_id <> 100 " + " and pv.portfolio_id = " + portfolioID + ") ");
	...
	/**
	 * Finds all ProjectSpaces for the specified portfolioID, filtering out any
	 * projects dependent on the specified projectID filter. This will create a
	 * list of non-dependent projects.
	 * 
	 * @param portfolioID
	 *            the ID of the portfolio to load
	 * @param projectIDFilter
	 *            the id of the project to filter decendents from
	 * @return a List where each element is a <code>ProjectSpace</code>
	 * @throws PersistenceException
	 *             if there is a problem loading the ProjectSpaces
	 * @throws NullPointerException
	 *             if the portfolioID is null
	 */
	public final List<ProjectSpace> findNonDependentsByPorfolioID(String portfolioID, String projectIDFilter) throws PersistenceException {

		if (portfolioID == null) {
			throw new NullPointerException("portfolioID is required");
		}

		String cycleExclusionFilterSQL = "(select child_space_id from pn_space_has_space " + "where relationship_parent_to_child = 'superspace' "
				+ "start with parent_space_id = " + projectIDFilter + " connect by prior child_space_id = parent_space_id)";
	 ...
	 *            the id of the business space to find
	 * @return a List where each element is a <code>ProjectSpace</code>.
	 * @throws PersistenceException
	 *             if there is a problem loading the ProjectSpace
	 * @throws NullPointerException
	 *             if the specified businessSpaceID is null
	 */
	...
	/**
	 * Returns a list of project spaces for a business. Only the ones belonging to that business.
	 * 
	 * @param businessSpaceID
	 *            the id of the owner business.
	 * @return a list of project spaces.
	 * @throws PersistenceException
	 *             in case of an error on the database.
	 */
	public final List<ProjectSpace> findProjectSpacesByBusinessID(String businessSpaceID) throws PersistenceException {
		if (businessSpaceID == null) {
			throw new NullPointerException("businessSpaceID cannot be null");
		}

		List<ProjectSpace> projectSpaceList = new ArrayList<ProjectSpace>();
		DBBean db = new DBBean();

		try {
			// Execute the select statement
			ResultSet result = null;
			result = fetchResultSetByBusinessIDOnlyForThatBusiness(db, businessSpaceID);
			...
			Logger.getLogger(ProjectSpaceFinder.class).error("Error loading project space by business id: " + sqle);
			throw new PersistenceException("ProjectSpace find by business id operation failed: " + sqle, sqle);
	 ...
	 *            the id of the project space to find
	 * @return a List where each element is a <code>ProjectSpace</code>.
	 * @throws PersistenceException
	 *             if there is a problem loading the ProjectSpace
	 * @throws NullPointerException
	 *             if the specified projectSpaceID is null
	 */
	public final List<ProjectSpace> findByID(String projectSpaceID) throws PersistenceException {
	...
	/**
	 * Finds and loads a project space for the specified id. Populates the
	 * specified project space.
	 * 
	 * @param projectSpaceID
	 *            the id of the project space to load
	 * @param projectSpace
	 *            the project space to populate
	 * @return true if the project space was found; false otherwise
	 * @throws PersistenceException
	 *             if there is a problem loading the ProjectSpace
	 * @throws NullPointerException
	 *             if the specified projectSpaceID or projectSpace is null
	 */
	 ...
	 * 
	 * @param projectSpaceIDCollection
	 *            the colllection of <code>String</code> ids of the project
	 *            spaces to find
	 * @return a List where each element is a <code>ProjectSpace</code>. The
	 *         number of entries in the list will match the number of entries in
	 *         the specified collection, assuming each ID in the collection
	 *         represents a project ID. If any IDs are not found, no exception
	 *         is thrown; rather, the returned list will have fewer entries.
	 * @throws PersistenceException
	 *             if there is a problem loading the ProjectSpaces
	 * @throws NullPointerException
	 *             if the specified projectSpaceIDCollection is null or empty
	 */
	public final List<ProjectSpace> findByIDs(Collection<String> projectSpaceIDCollection) throws PersistenceException {
	 ...
	 * 
	 * @param projectSpaceIDCollection
	 *            the collection of <code>String</code> id's of the project
	 *            spaces to find
	 * @param recordStatus
	 *            the record status of projects to find; if null, no filter on
	 *            record status will be applied
	 * @return a List where each element is a <code>ProjectSpace</code>. The
	 *         number of entries in the list will match the number of entries in
	 *         the specified collection, assuming each ID in the collection
	 *         represents a project ID and has a matching record status. If any
	 *         IDs are not found, no exception is thrown; rather, the returned
	 *         list will have fewer entries.
	 * @throws PersistenceException
	 *             if there is a problem loading the ProjectSpaces
	 * @throws NullPointerException
	 *             if the specified projectSpaceIDCollection is null or empty
	 */
	public final List<ProjectSpace> findByIDs(Collection<String> projectSpaceIDCollection, String recordStatus) throws PersistenceException {
		...
		int counter = 1;
		for (Iterator<String> it = projectSpaceIDCollection.iterator(); it.hasNext(); counter++) {
	 ...
	 * 
	 * @param recordStatus
	 *            the record status of ProjectSpaces to find
	 * @return a list where each element is a <code>ProjectSpace</code>; the
	 *         list is currently orderd by name, ordering performed by database
	 * @throws PersistenceException
	 *             if there is a problem loading the ProjectSpaces
	 * @throws NullPointerException
	 *             if the record status is null
	 */
	public final List<ProjectSpace> findAllForRecordStatus(String recordStatus) throws PersistenceException {
	...
	/**
	 * Finds all <code>ProjectSpace</code>s in the database based on part of a
	 * project name.
	 * 
	 * @param recordStatus
	 *            the record status of ProjectSpaces to find
	 * @param nameFilter
	 *            part of the project name; A case insensitive search is
	 *            performed.
	 * @return a list where each element is a <code>ProjectSpace</code>; the
	 *         list is currently ordered by name, ordering performed by database
	 * @throws PersistenceException
	 *             if there is a problem loading the ProjectSpaces
	 * @throws NullPointerException
	 *             if the record status is null
	 */
	public final List<ProjectSpace> findAllByName(String recordStatus, String nameFilter) throws PersistenceException {
	...
	/**
	 * Executes a select statement to find ProjectSpace for the specified id. Fetches projects 
	 * and sub-projects belonging to this or child sub-business.
	 * 
	 * @param db
	 *            the DBBean in which to perform the transaction.
	 * @param businessSpaceID
	 *            the id of the business space to load projects from.
	...
	private ResultSet fetchResultSetByBusinessID(DBBean db, String businessSpaceID) throws SQLException {
		...
		StringBuffer query = new StringBuffer();
		query.append(getBaseSQLStatement());
		// this gets all the projects and sub projects under a root business and
		// its sub businesses
		query.append("and p.project_id in (");
	...
	/**
	 * Executes a select statement to obtain all the projects from that business
	 * space.
	 * 
	 * @param db
	 *            the DBBean in which to perform the transaction.
	 ...
	 *             if there is a problem accessing the database.
	 */
	private ResultSet fetchResultSetByBusinessIDOnlyForThatBusiness(DBBean db, String businessSpaceID)throws SQLException {
		...
		query.append("select child_space_id from pn_space_has_space where ");
		query.append("record_status = 'A' and child_space_type = 'project' ");
		query.append("and parent_space_id = ?)");
//		query.append("start with parent_space_id = ? or parent_space_id in (");
//		query.append("select child_space_id from pn_space_has_space where ");
//		query.append("record_status = 'A' and child_space_type = 'business' ");
//		query.append("start with parent_space_id = ? connect by parent_space_id = prior child_space_id) ");
...
//		db.pstmt.setString(++index, businessSpaceID);
	...
	/**
	 * Populates the specified project space from a result set row. Assumes
	 * result contains a current row.
	 * 
	 * @param result
	 *            the result set from which to populate
	 * @param projectSpace
	 *            the project space to populate
	 * @throws SQLException
	 *             if there is a problem getting a value from the result set
	 */
	private static void populate(java.sql.ResultSet result, ProjectSpace projectSpace) throws SQLException {
		...
		try {
			...
			projectSpace
					.setCurrentEstimatedTotalCost(makeMoney(result.getString("current_est_total_cost_value"), result.getString("current_est_total_cost_cc")));
			...
			// calculates this project's percent complete based on the
			// calculation method loaded
			projectSpace.calculatePercentComplete();
			// nulling the metadata, so it will be reloaded on the first request
			// (getMetaData call)
	 ...
	 * @param currencyCode
	 *            the currency code
	 * @return the parsed Money value or null if either the value or
	 *         currencyCode are null
	 * @throws InvalidValueException
	 *             if the value is present but not valid
	 * @throws InvalidCurrencyException
	 *             if the currencyCode is specified but not valid
	 */
	...
	/**
	 * Executes a select statement to find minimal project space for the
	...
	/**
	 * Get parent business name by parent space id of child project
	 * 
	 * @param parentSpaceId
	 *            the parent space id of child project
