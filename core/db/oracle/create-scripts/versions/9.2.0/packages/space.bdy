...
BEGIN
    ...
    -- and create a "Principle Group" for this space
	 ...
	 FOR group_record IN (SELECT g.group_id g, i_person_id
	            FROM pn_group g, pn_space_has_group sg
	            WHERE space_id = i_space_id
	              AND g.group_type_id = SECURITY.GROUP_TYPE_TEAMMEMBER
	              AND g.group_id = sg.group_id) LOOP
	     
	     SELECT count(group_id) INTO v_groups FROM pn_group_has_person WHERE group_id = group_record.g and person_id = i_person_id;
	      
	     IF v_groups = 0 THEN 
	       INSERT INTO pn_group_has_person  (group_id, person_id) 
	       VALUES (group_record.g, i_person_id);
	     END IF;
