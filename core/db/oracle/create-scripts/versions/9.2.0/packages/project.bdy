...
BEGIN
    ...
    ****/
    ...
    FROM
        pn_module
    WHERE
        module_id IN (10, 20, 30, 40, 60, 70, 90, 100, 110, 120, 140, 150, 180, 190, 200, 210, 260, 310, 330, 340, 380);
    ...
    *****************************************************************/
    ...
    -- add plan default
    SCHEDULE.STORE_PLAN(p_plan_name, 'Main Project Plan', p_start_date, p_end_date, p_autocalc_schedule, null, null, v_creator, v_project_id, null, null, null, null, null, 50, p_start_date, 1, 0, 8, 40, 20, 'PERSONAL', v_plan_id);
...
BEGIN
	...
	/*  BFD 1064 */
	-- check whether the existing parent business is being removed or changed
	IF ( (v_current_business > 0 AND v_business_space_id is null) 
		OR (v_current_business <> v_business_space_id) ) THEN
	  
	    -- get all inherited groups of business to remove
	    for current_business_groups in ( select business_groups.group_id 
			  from pn_space_has_group business_groups, pn_space_has_group project_groups
			  where business_groups.group_id = project_groups.group_id 
			  and business_groups.space_id = v_current_business 
			  and project_groups.space_id = v_project_id ) loop
	      begin
	      
		-- remove the inherited business group by project space
		SECURITY.remove_inherited_group(v_project_id, current_business_groups.group_id);
