...
-- Nico		   07-Sep-13  Added Module Material (260)
...
****************************************************************************************************/
    ...
    FROM
        pn_module
    WHERE
        module_id IN (10, 20, 30, 70, 110, 140, 170, 180, 190, 200, 210, 260, 310, 330, 340, 390);
...
BEGIN
    ...
    select count (space_id) into v_num_projects
        from pn_portfolio_has_space pphs, pn_project_space pps
        where pphs.portfolio_id = v_portfolio_id
        and pphs.space_id = pps.project_id
        and pps.record_status = 'A';
